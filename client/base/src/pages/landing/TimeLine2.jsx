  ...
  return (
    <div className='w-[100%] flex items-center justify-center font-[inter] tracking-wide'>
        ...
        <div id="" className='flex gap-3 font-bold'>
          {Object.keys(data).map((item,index)=><div className={`flex-1 flex gap-2 flex-col bg-white ${item==selectedCard?"bg-white":"opacity-50"} p-4 rounded-t text-xl` }onClick={()=>{setSelectedCard(item);setSelectedIndex(index)}}>{`${index+1}. ${item}`}{item==selectedCard?<Progress/>:<LinearProgress variant="determinate" sx={{height:2}}  />}</div>)}
        </div>
        <div className='bg-white flex pb-5 flex justify-between'>
          ...
          <div id="image" className='flex items-center max-w-[300px] pr-5' >
          {selectedCard=="signup" ? (
